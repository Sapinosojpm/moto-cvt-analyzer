...
export default function RecommendationPanel({ recommendation }: RecommendationPanelProps) {
  const getColorClass = (rec: string) => {
    ...
    if (rec.includes("SLIPPING") || rec.includes("HIGH LOAD") || rec.includes("SUBOPTIMAL")) return "bg-red-900 border-red-500 text-red-100";
  ...
  return (
    <div className={`border-l-4 rounded-lg p-4 ${getColorClass(recommendation)}`}>
      <h3 className="text-sm font-medium uppercase tracking-wide">CVT Tuning Advisor</h3>
